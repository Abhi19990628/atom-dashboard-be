...
from api.serializers import (
    ...
    ProcessAuditChecksheetSerializer,
    CoherenceChecklistSerializer,
    LayoutInspectionSerializer,
    ProductAuditPlanSerializer,
    CustomerComplaintSerializer,
    CustomerSatisfactionSerializer,
    WarrantyClaimSerializer,
    MinutesOfMeetingSerializer,
    IncomingMaterialInspectionSerializer,
)
...
class MasterDropdownView(APIView):
    def get(self, request):
        ...
        if filter_type == "customer":
            data = L1_PartInfoMaster.objects.values_list(
            ...
            ops = (
                L2_ProcessReportMaster.objects.filter(part_info__part_name=part)
            ...
            data = (
                L1_PartInfoMaster.objects.filter(customer_name=cust)
            ...
            part = request.query_params.get("part")
            ops = (
                L2_ProcessReportMaster.objects.filter(
                    part_info__customer_name=cust, part_info__part_name=part
                )
                .values_list("report_name", flat=True)
                .distinct()
            )
            ...
            part = request.query_params.get("part")
            data = (
                L1_PartInfoMaster.objects.filter(part_name=part)
            ...
            data = L3_ParameterDetailMaster.objects.values_list(
                "instrument", flat=True
            ).distinct()
            clean_data = sorted(
                list(set([str(x).strip() for x in data if x and str(x).strip()]))
            )
            return Response(clean_data)
        elif filter_type == "parameter":
            data = L3_ParameterDetailMaster.objects.values_list(
                "parameter_name", flat=True
            ).distinct()
            clean_data = sorted(
                list(set([str(x).strip() for x in data if x and str(x).strip()]))
            )
...
class MasterParametersView(APIView):
    def get(self, request):
        ...
        params = L3_ParameterDetailMaster.objects.filter(
    ...
    def post(self, request):
        try:
            ...
            for row in items:
                entries_to_create.append(
                    RedBinAnalysisReport(
                        ...
                        part_name_model=row.get("part_name_model", "")
            ...
            return Response(
                {
                    "success": True,
                    "message": "Saved EXACTLY as AM/PM!",
    ...
    def post(self, request):
        try:
            ...
            report = DeviationApproval.objects.create(
                tool_name_no=data.get("tool_name_no", ""),
                location=data.get("location", ""),
                problem=data.get("problem", ""),
                reason_for_deviation=data.get("reason_for_deviation", ""),
                date=data.get("date"),
                duration=data.get("duration", ""),
                prod_incharge=data.get("prod_incharge", ""),
                qa_incharge=data.get("qa_incharge", ""),
                remarks=data.get("remarks", ""),
            )
            return Response(
                {
                    "success": True,
                    "message": "Deviation Approval Data Saved!",
                    "record_id": report.id,
                },
                status=status.HTTP_201_CREATED,
            )
    ...
    def post(self, request):
        try:
            ...
            return Response(
                {
                    "success": True,
                    "message": "Material Requisition Slip Saved!",
                    "record_id": report.id,
                },
                status=status.HTTP_201_CREATED,
            )
    ...
    def post(self, request):
        try:
            ...
            if master:
                customer_account = (
                    master.get("customer")
                    or master.get("customer_account")
                    or "Unknown"
                )
                ...
                customer_account = (
                    data.get("customer_account") or data.get("customer") or "Unknown"
                )
        ...
        try:
            ...
            mapped_data = {
                ...
                "performance_indicators": {
                    ...
                    "premium_freight_incidents": raw_data.get(
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                return Response(
                    {
                        "success": True,
                        "message": "Incoming Material Inspection Saved!",
                        "record_id": obj.id,
                    },
                    status=status.HTTP_201_CREATED,
                )
...
class GetInspectionReportView(APIView):
    def get(self, request):
        ...
        filters = {}
        if customer:
            filters["customer_account__icontains"] = customer
        if part_name:
            filters["part_name__icontains"] = part_name
        if operation:
            filters["operation__icontains"] = operation
        if date:
        ...
        if reports.exists():
            ...
            return Response(
                {"message": "No report found for given filters"},
    ...
    try:
        ...
        def format_rework(r):
            ...
            final_status = (
                "OK"
                if status_val == "ok"
                else "NOT OK" if status_val == "notok" else "—"
            )
        ...
        FORM_REGISTRY = {
            "deviation": lambda r: {
                "toolNameNo": r.tool_name_no,
                "location": r.location,
                "problem": r.problem,
                "reasonForDeviation": r.reason_for_deviation,
                "date": str(r.date),
                "duration": r.duration,
                "prodIncharge": r.prod_incharge,
                "qaIncharge": r.qa_incharge,
                "remarks": r.remarks,
            },
            "deviation-view": lambda r: FORM_REGISTRY["deviation"](r),
            "good-receipt": lambda r: {
            ...
            "redbin": lambda r: {
                ...
                "part_name_model": r.part_name_model,
                "operation": r.operation,
                "total_rej_qty": r.total_rej_qty,
                ...
                "completion_date": str(r.completion_date) if r.completion_date else "",
            },
            ...
            "process-audit": lambda r: {
                "part_name_no": r.part_name_no,
                "model_name": r.machine_model,
                "audit_date": str(r.date),
                "auditor_name": r.auditor,
                "auditee_name": r.auditee,
            ...
            "customer-complaint": lambda r: {
                ...
                "horizontal_action": r.horizontal_action,
                "status": r.status,
            },
            "customer-complaint-view": lambda r: FORM_REGISTRY["customer-complaint"](r),
            "customer-satisfaction": lambda r: {
                "customerName": r.customer_name,
                "monthYear": r.month_year,
                **r.performance_indicators,
            },
            "customer-satisfaction-view": lambda r: FORM_REGISTRY[
                "customer-satisfaction"
            ](r),
            "warranty-claim": lambda r: {
                "date": str(r.date),
                "customerName": r.customer_name,
                "partDetails": r.part_details,
                "claimQty": r.claim_qty,
                "warrantyDefect": r.warranty_defect,
                "decision": r.decision,
                "rejectionRootCause": r.rejection_root_cause,
                "disposalAction": r.disposal_action,
                "capaAnalysis": r.capa_analysis,
            },
            ...
            "mom": lambda r: {
                ...
                "time": str(r.time),
                "subject": r.subject,
                "aotMembers": r.aot_members,
                "supplierMembers": r.supplier_members,
                "discussions": r.discussions,
            },
            "mom-view": lambda r: FORM_REGISTRY["mom"](r),
            "redbin-attendance": lambda r: {
                "date": str(r.date),
                "month": r.month,
                "year": r.year,
        ...
        MODEL_REGISTRY = {
            ...
            "deviation-view": DeviationApproval,
            "good-receipt": GoodReceiptEntry,
            "incoming": IncomingMaterialInspection,
            "incoming-inspection-view": IncomingMaterialInspection,
            "redbin": RedBinAnalysisReport,
            "redbin-view": RedBinAnalysisReport,
            "scrap": ScrapNoteEntry,
            "scrap-note-view": ScrapNoteEntry,
            "process-audit": ProcessAuditChecksheet,
            "process-audit-view": ProcessAuditChecksheet,
            "coherence": CoherenceChecklist,
            "coherence-view": CoherenceChecklist,
            "layout-inspection": LayoutInspection,
            "layout-inspection-view": LayoutInspection,
            "product-audit-plan": ProductAuditPlan,
            "product-audit-plan-view": ProductAuditPlan,
            "customer-complaint": CustomerComplaint,
            "customer-complaint-view": CustomerComplaint,
            "customer-satisfaction": CustomerSatisfaction,
            "customer-satisfaction-view": CustomerSatisfaction,
            "warranty-claim": WarrantyClaim,
            "warranty-claim-view": WarrantyClaim,
            "mom": MinutesOfMeeting,
            "mom-view": MinutesOfMeeting,
            "redbin-attendance": RedBinAttendance,
            "redbin-attendance-view": RedBinAttendance,
            "inspection": InspectionReport,
            "inspection-view": InspectionReport,
            "rework": ReworkEntry,
        ...
        if form_key not in FORM_REGISTRY or form_key not in MODEL_REGISTRY:
            return Response(
                {
                    "success": False,
                    "error": f"Form '{form_key}' Not Supported Yet in QA",
                },
                status=400,
            )
...
def qa_data_view(request, form_key):
    ...
    def format_to_ampm(raw_time):
        ...
        try:
            if hasattr(raw_time, "strftime"):
                ...
                local_dt = (
                    localtime(raw_time, ist_tz)
                    if getattr(raw_time, "tzinfo", None)
                    else raw_time
                )
        ...
        with connection.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                if status and status.startswith("Approved"):
                    ...
                    approver_name = (
                        raw_approver.split("@")[0].capitalize() if raw_approver else "—"
                    )
                ...
                for row in cursor.fetchall():
                    ...
                    for param in parameters:
                        ...
                        row_data = {
                            ...
                            "Prepared By": log_mapping.get(rec.get("id"), {}).get(
                                "prepared_by", "—"
                            ),
                            "Approved By": log_mapping.get(rec.get("id"), {}).get(
                                "approved_by", "—"
                            ),
                        }
                        ...
                        for log in logs:
                            stage_name = log.get(
                                "displayStage", log.get("baseStage", "STAGE")
                            ).upper()
    ...
    # ── 3. RED BIN ANALYSIS ────────────────────────────────────
        ...
        try:
            ...
            reports = apply_date_filter(base_query, "entry_date").order_by(
                "-entry_date", "-created_time"
            )
            data = []

            for report in reports:
                raw_time = getattr(
                    report, "created_time", getattr(report, "created_at", None)
                )
    ...
    # ── 4. RED BIN ATTENDANCE ──────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                RedBinAttendance.objects.all(), "date"
    ...
    # ── 5. SCRAP NOTE ──────────────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                ScrapNoteEntry.objects.all(), "entry_date"
    ...
    # ── 6. REWORK REPORT ───────────────────────────────────────
        ...
        try:
            reports = apply_date_filter(ReworkEntry.objects.all(), "date").order_by(
                "-date", "-created_at"
            )
            data = []
            for r in reports:
    ...
    # ── 7. DEVIATION APPROVAL ──────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                DeviationApproval.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                data.append(
    ...
    # ── 8. GOOD RECEIPT ENTRY (NEW) ────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                GoodReceiptEntry.objects.all(), "received_date"
    ...
    # ── 9. PROCESS AUDIT CHECKSHEET ────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                ProcessAuditChecksheet.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                base_info = {
    ...
    # ── 10. COHERENCE CHECKLIST ─────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                CoherenceChecklist.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                base_info = {
                ...
                if not operations:
                    data.append(
                        {**base_info, "Operation Detail": "No operations added"}
                    )
                else:
    ...
    # ── 11. LAYOUT INSPECTION ───────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                LayoutInspection.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                base_info = {
                ...
                if not inspections:
                    data.append(
                        {**base_info, "Inspection Detail": "No inspections added"}
                    )
                else:
    ...
    # ── 12. PRODUCT AUDIT PLAN ──────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                ProductAuditPlan.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                base_info = {
    ...
    # ── 13. CUSTOMER COMPLAINT ──────────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                CustomerComplaint.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                data.append(
                    {
                        ...
                        "Target Date": (
                            str(report.target_date) if report.target_date else "—"
                        ),
    ...
    # ── 14. CUSTOMER SATISFACTION ───────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                CustomerSatisfaction.objects.all(), "created_at"
            ).order_by("-created_at")
            data = []
            for report in reports:
                row = {
    ...
    # ── 15. WARRANTY CLAIM ──────────────────────────────────────
        ...
        try:
            reports = apply_date_filter(WarrantyClaim.objects.all(), "date").order_by(
                "-date", "-created_at"
            )
            data = []
            for report in reports:
    ...
    # ── 16. MINUTES OF MEETING (MOM) ────────────────────────────
        ...
        try:
            reports = apply_date_filter(
                MinutesOfMeeting.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                base_info = {
    ...
    # ── 17. INCOMING MATERIAL INSPECTION (NEW) ──────────────────
        ...
        try:
            reports = apply_date_filter(
                IncomingMaterialInspection.objects.all(), "date"
            ).order_by("-date", "-created_at")
            data = []
            for report in reports:
                base_info = {
