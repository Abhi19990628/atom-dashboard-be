...
class Plant2ExactRequirementState:
    def __init__(self):
        ...
        # POWER / J-SIGNAL:
        # Physical OFFLINE must not wait for the 3-minute ONLINE-IDLE threshold.
        # J messages are frequent and the monitor checks every 5 seconds.
    ...
    def _mark_idle_notification_ended(
        ...
        try:
            ...
            #
            # "Plant 2 Machine M-XX is IDLE/OFFLINE.
    ...
    def save_resolved_downtime_to_db(
        ...
        with self.lock:
            if idle_mins > 0:
                ...
                with self.reason_lock:
                    pending_data = dict(self.pending_reasons.get(machine_no) or {})
...
def auto_generate_idle_notification(machine_no, idle_mins):
    ...
    try:
        ...
        if ideal_mode == "OFFLINE":
            message = (
                f"Plant 2 Machine M-{int(machine_no):02d} "
                f"is OFFLINE. "
                f"Please submit the downtime reason."
            )
        else:
        ...
        print(f"❌ Plant 2 Auto Alert Error " f"M{machine_no}: {e}")
        traceback.print_exc()
        return False
