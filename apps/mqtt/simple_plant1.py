...
from api.models import Notification, IdealTimeSegmentReason
...
class StrictIdlePolicy:
    ...
    def start_new_on_session(self, m: int, t: datetime):
        ...
        current ONLINE session me carry nahi hona chahiye.
        """

        with self.lock:
            now = self._ist(t)

            # New physical ON session starts NOW
            self.on_since[m] = now

            # Old session timing current ONLINE idle me use nahi hogi
            self.last_count_time.pop(m, None)
            self.last_json_time.pop(m, None)

            # Current new session ka accumulated idle fresh start
            self.completed_segments_minutes[m] = 0
...
class Plant1ExactRequirementState:
    def __init__(self):
        ...
        self.last_hour_counts = defaultdict(int)
        self.shift_cumulative = defaultdict(int)
        self.shift_cumulative_loaded = (
            set()
        )  # ✅ Batch insert ke liye DB se cumulative once-per-shift load hoga
        self.current_hours = {}
        self.current_shifts = {}
    ...
    def set_pending_reason(
        ...
        print(
            f"📝 Buffer Updated for M{machine_no}: "
            f"{category} -> {reason} | "
            f"User={submitted_by or '-'} | "
            f"IdealID={ideal_event_id or '-'}"
        )
    ...
    def _get_ideal_reason_data(
        ...
        """
        IdealTimeSegmentReason is the permanent source of truth.

        This helper is only for initial/default data while an Ideal
        event is being created/saved.

        Priority:
        1. Current RAM pending reason
        2. Default PENDING reason

        Notification table se reason recover nahi karna.
        """
    ...
    def _save_ideal_piece_to_db(
        ...
        try:
            ...
            if (
                ...
                print(
                    f"⏭️ ONLINE IDEAL IGNORED | "
                    f"M{machine_no} | "
                    f"{start_at.strftime('%H:%M:%S')}"
                    f"→{end_at.strftime('%H:%M:%S')} | "
                    f"{ideal_seconds}s < "
                    f"{self.online_idle_threshold_seconds}s"
                )
                return False
            ...
            # Timezone remove nahi karna hai.
            start_db = self._as_ist(start_at)
            end_db = self._as_ist(end_at)

            # ✅ FINAL OVERLAP GUARD:
            # Same machine me same time par ONLINE/OFFLINE dono rows nahi banengi.
            overlap = self._db_has_ideal_overlap(machine_no, start_db, end_db)
            if overlap:
                print(
    ...
    def _save_ideal_range_split_by_hour(
        ...
        ONLINE:
            One physical event = one DB row.
            Minimum qualification = 180 seconds.
            Never hour-split.
    
        OFFLINE:
            No 180-second threshold.
            Physically split on every clock-hour boundary.
    
        Example:
            08:30 -> 09:00 HOUR_CHANGE
        ...
        try:
            ...
            #
            # No 3-minute threshold.
            # Split physically at exact clock-hour boundaries.
            # ======================================================
    ...
    def _start_ideal_segment(self, machine_no, ideal_mode, start_at):
        ...
        if ideal_mode == "OFFLINE":
            ...
            try:
                ...
                shift_start = self.get_shift_start_datetime(start_at)
                ...
                ideal_event = (
                    ...
                    .filter(
                        ...
                        ideal_start_at__gte=shift_start,
                    )
                    .order_by(
                        "-ideal_start_at",
                        "-id",
                ...
                # First row of one continuous OFFLINE incident.
                active.setdefault(
                    "canonical_event_id",
                    ideal_event.id,
                )
    ...
    def _mark_idle_notification_ended(
        ...
        """
        Machine resume/ON hone ke baad exact Ideal event ka
        final duration notification message me update karta hai.

        IMPORTANT:
        - created_at ko change nahi karna.
        - created_at = notification creation time.
        - final duration IdealTimeSegmentReason se aayega.
        """
        ...
        try:
            ...
            # ------------------------------------------------------
            # Exact Ideal ID compulsory
            # ------------------------------------------------------
            if not ideal_event_id:
                print(
                    f"⚠️ Notification message update skipped | "
                    f"M{machine_no} | Ideal ID missing",
                    flush=True,
                )
                return 0
            ...
            # VERY IMPORTANT
            #
            # Reason form NOT submitted yet:
            #
            # user_id = NULL
            #
            # In this case message MUST remain:
            #
            # "Plant 1 Machine M-XX is IDLE/OFFLINE.
            #  Please submit the downtime reason."
            #
            # Machine close must NOT add duration before form submit.
            # ======================================================
            ...
            if (
                ...
                print(
                    f"ℹ️ NOTIFICATION MESSAGE KEPT PENDING | "
                    f"Plant 1 | "
                    f"M{machine_no} | "
                    f"IdealID={ideal_event_id} | "
                    f"ReasonNotSubmitted",
                    flush=True,
                )
            ...
            if (
                ...
                final_message = (
                    ...
                    f"Machine "
                    f"M-{int(ideal_event.machine_no):02d} "
                ...
                final_message = (
                    ...
                    f"was {status_text} for "
                    f"{duration_text}. "
                    f"Please submit the downtime reason."
                )
            ...
            print(
                f"✅ NOTIFICATION FINAL MESSAGE UPDATED | "
                f"{ideal_event.plant_location} | "
                f"M{ideal_event.machine_no} | "
                f"IdealID={ideal_event_id} | "
    ...
    def _close_ideal_segment(
        ...
        """
        OPTIMIZED FINAL CLOSE:

        If 3+ min notification ke time OPEN Ideal row
        already create hui hai:
            SAME ROW update hogi.

        Agar open row available nahi hai:
            legacy/fallback closed-row save chalega.
        ...
        try:
            ...
            # ======================================================
            # 2. OPEN ROW EXISTS
            #    → SAME ROW CLOSE / UPDATE
            # ======================================================

            if ideal_event is not None:

                db_start_at = self._as_ist(
                    ideal_event.ideal_start_at
                )
                ...
                if final_seconds >= required_seconds:
                    ...
                    print(
                        ...
                        f"M{machine_no} | "
                        f"IdealID={ideal_event.id} | "
                        f"{final_seconds}s < {required_seconds}s",
                        flush=True,
                    )
            ...
            # ======================================================
                ...
                # ==============================================
                # RECOVERY USING NOTIFICATION START
                # ==============================================
                ...
                if (
                    ...
                    print(
                        f"⚠️ NORMAL IDEAL SAVE MISSED | "
                        f"Plant 1 | "
                        f"M{machine_no} | "
                        f"{ideal_mode} | "
                        f"Retry using notification time "
                        f"{notification_start_at.strftime('%H:%M:%S')}"
                        f"→{end_at.strftime('%H:%M:%S')}",
                        flush=True,
                    )
        ...
        # ==========================================================
        # 4. CLOSE OLD NOTIFICATION TIMING FIELD
        #
        # Temporary compatibility.
        # Final normalization me idle_ended_at remove hoga.
        # ==========================================================

        if saved > 0:
                
            notification_event_start_at = (
                notification_start_at
                or start_at
            )
            ...
            print(
                f"❌ IDEAL NOT SAVED - "
                f"NOTIFICATION NOT CLOSED | "
                f"Plant 1 | "
                f"M{machine_no} | "
                f"{ideal_mode} | "
                f"{exact_start_at.strftime('%H:%M:%S')}"
                f"→{end_at.strftime('%H:%M:%S')} | "
                f"IdealID={ideal_event_id or '-'}",
                flush=True,
            )
    ...
    def close_ideal_on_count_resume(self, machine_no, now_ist):
        ...
        if machine_no not in self.active_ideal_segments:
            ...
            if machine_no in self.machine_count_status:
                ...
                if (now_ist - start_at).total_seconds() >= self.online_idle_threshold_seconds:
                    self._start_ideal_segment(machine_no, "ONLINE", start_at)
            elif machine_no in self.machine_on_since:
                on_since = self.machine_on_since[machine_no]
    ...
    def split_active_ideal_segment_at_hour(
        self,
        machine_no,
        now_ist,
    ):
        ...
        - Split physically on exact clock-hour boundaries.
        - ONLINE is NEVER split here.
        - Restart safe / idempotent:
          if an exact hourly DB row already exists,
          reuse it instead of inserting/updating a duplicate.
        ...
        try:
            ...
            if active.get("mode") != "OFFLINE":
                return

            active.setdefault(
                "event_started_at",
                self._as_ist(active["start_at"]),
            )
            ...
            while True:
                ...
                # ======================================================
                # CURRENT HOUR HAS NOT FINISHED YET
                #
                # Ensure exactly one current OPEN tail exists.
                # ======================================================
                if now_ist < next_hour:

                    current_event = None

                    current_event_id = active.get(
                        "ideal_event_id"
                    )
                    ...
                    # No OPEN row exists for the current tail.
                    if current_event is None:
                        ...
                        if previous_event is not None:
                            ...
                            specific_reason = (
                                previous_event.specific_reason
                            )
                            remark = previous_event.remark
                            report_status = (
                                previous_event.report_status
                            )
                            submitted_by = (
                                previous_event.submitted_by
                            )
                            submitted_at = (
                                previous_event.submitted_at
                            )
                        ...
                        current_event = (
                            ...
                            .create(
                                ...
                                specific_reason=specific_reason,
                                remark=remark,
                                shift=self.get_shift_from_time(
                                    start_at
                                ),
                                report_status=report_status,
                                submitted_by=submitted_by,
                        ...
                        print(
                            f"🔴 OFFLINE OPEN TAIL SAVED | "
                            f"Plant 1 | "
                            f"M{machine_no} | "
                            f"IdealID={current_event.id} | "
                            f"Start={start_at.strftime('%H:%M:%S')}",
                            flush=True,
                        )
                ...
                # HOUR HAS FINISHED
                #
                # FIRST check whether exact physical piece already
                # exists in DB.
                #
                # This is the restart-safe duplicate protection.
                # ======================================================
                ...
                if existing_closed is not None:
                    ...
                    previous_event = existing_closed
                    ...
                    print(
                        f"♻️ OFFLINE HOUR RECOVERED | "
                        f"Plant 1 | "
                        f"M{machine_no} | "
                        f"IdealID={existing_closed.id} | "
                        f"{start_at.strftime('%H:%M:%S')}"
                    ...
                    # ==================================================
                    # Exact closed row does NOT exist.
                    # Find the exact OPEN row for this hour.
                    # ==================================================
                ...
                # ======================================================
                # Move RAM pointer forward.
                #
                # IMPORTANT:
                # Do NOT immediately create historical OPEN rows.
                # Next loop first checks whether that closed hour
                # already exists.
                # ======================================================

                active["start_at"] = next_hour
                active["ideal_event_id"] = None

        except Exception as e:

            print(
                f"❌ OFFLINE HOUR SPLIT ERROR | "
                f"Plant 1 | "
                f"M{machine_no} | {e}",
                flush=True,
            )
           ...
           # ==========================================================
            ...
            if was_offline_before_signal:
                ...
                if (
                    ...
                    offline_start = (
                        self._infer_offline_start_for_new_signal(
                            now_ist
                        )
                    )
        ...
        with self.lock:
            ...
            if shift_key not in self.shift_cumulative_loaded:
                self.shift_cumulative[shift_key] = self._load_last_cumulative_from_db(
                    machine_no=machine_no, shift=current_shift, timestamp=now_ist
                )
        ...
        try:
            ...
            if cumulative_count is None:
                cumulative_count = int(
                    self.shift_cumulative.get((machine_no, shift), 0)
                )
    ...
    def bulk_insert_realtime_counts(self, rows):
        ...
        print(
            f"❌ Plant1 batch insert failed permanently | rows={len(rows)} | error={last_error}"
        )
        raise last_error
        ...
        with self.lock:
            ...
            if machine_no in self.machine_count_status:
                ...
                recent_count_signal = (
                    ...
                    and
                    count_seconds_ago
                    <= self.power_signal_timeout_seconds
                )
...
def auto_generate_idle_notification(machine_no, idle_mins):
    ...
    3+ minute actual physical Ideal hone par:

    1. Main IdealTimeSegmentReason OPEN row create/find karega.
    2. Same physical event ka event_key attach karega.
    3. Notifications ko ideal_event FK se connect karega.

    ONE PHYSICAL IDLE EVENT = ONE IDEAL ROW.
    """
    ...
    try:
        ...
        if idle_started_at is None:
            ...
            print(
                ...
                f"Plant 1 | "
                f"M{machine_no} | "
                f"Active Ideal has no start time",
                flush=True,
            )
        ...
        #
        # event_key is no longer required.
        #
        # Primary identity:
        #     IdealTimeSegmentReason.id
        #
        # RAM keeps:
        ...
        # ----------------------------------------------------------
        # 2. Recovery:
        # Find same currently-open physical event.
        #
        # ±2 seconds because DB timestamps are now stored
        ...
        if ideal_event is None:
            ...
            print(
                ...
                f"{ideal_mode} | "
                f"IdealID={ideal_event.id} | "
                f"Start={idle_started_at.strftime('%H:%M:%S')}",
                flush=True,
            )
        ...
        if ideal_mode == "OFFLINE":
            message = (
                f"Plant 1 Machine M-{int(machine_no):02d} "
                f"is OFFLINE. "
                f"Please submit the downtime reason."
            )
        else:
        ...
        # ==========================================================
        # 6. ONE IDEAL EVENT = ONE NOTIFICATION
        #
        # Notification PK comes directly from ideal_event.
        #
        # At creation:
        #   user_id    = NULL
        #   created_at = NULL
        ...
        print(f"❌ Plant 1 Auto Alert Error " f"M{machine_no}: {e}")
        traceback.print_exc()
        return False
    ...
    try:
        ...
        while True:
            item = redis_client.rpoplpush(
                "plant1_processing_queue", "plant1_mqtt_queue"
            )
            if not item:
                break
            recovered += 1
...
def start_redis_queue_worker():
        ...
        while True:
            ...
            try:
                ...
                first_item = redis_client.brpoplpush(
                    "plant1_mqtt_queue", "plant1_processing_queue", timeout=1
                )
                ...
                while len(rows) < batch_size and time_module.time() < deadline:
                    next_item = redis_client.rpoplpush(
                        "plant1_mqtt_queue", "plant1_processing_queue"
                    )
                    if not next_item:
    ...
    if worker_count != 1:
        print(
            "⚠️ Plant 1 batch mode me PLANT1_QUEUE_WORKERS=1 recommended hai. Forcing to 1."
        )
        worker_count = 1
...
def on_disconnect(client, userdata, rc):
    if rc != 0:
        print(
            f"⚠️ Plant 1 MQTT disconnected unexpectedly. rc={rc}. Client will auto-reconnect."
        )
    else:
