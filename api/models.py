...
class Plant2HourlyIdletime(models.Model):
    ...
    shut_height = models.DecimalField(
...
class L2_ProcessReportMaster(models.Model):
    part_info = models.ForeignKey(
        L1_PartInfoMaster, related_name="process_reports", on_delete=models.CASCADE
    )
...
class L3_ParameterDetailMaster(models.Model):
    CATEGORY_CHOICES = [("PRODUCT", "PRODUCT"), ("PROCESS", "PROCESS")]

    process_report = models.ForeignKey(
        L2_ProcessReportMaster, related_name="parameters", on_delete=models.CASCADE
    )
...
class IncomingInspectionObservation(models.Model):
    inspection_report = models.ForeignKey(
        IncomingInspectionReport, on_delete=models.CASCADE, related_name="observations"
    )
...
class MachineChecksheetObservation(models.Model):
    report = models.ForeignKey(
        MachineChecksheetReport, on_delete=models.CASCADE, related_name="check_points"
    )
...
class DailyProductionReport(models.Model):
    ...
    part_info = models.ForeignKey(
        L1_PartInfoMaster, on_delete=models.RESTRICT, related_name="production_reports"
    )
    ...
    not_confirm_qty = models.IntegerField(
        default=0, verbose_name="Not Confirm / Hold Qty"
    )

    tool_setup_min = models.IntegerField(default=0)
    machine_bd_min = models.IntegerField(
        default=0, verbose_name="Machine Breakdown Min"
    )
    ...
    def save(self, *args, **kwargs):
        if self.ok_quantity == 0 and self.total_production > 0:
            self.ok_quantity = self.total_production - (
                self.rejection_qty + self.rework_qty + self.not_confirm_qty
            )
...
class FiveSChecksheetObservation(models.Model):
    report = models.ForeignKey(
        FiveSChecksheetReport, on_delete=models.CASCADE, related_name="observations"
    )
...
class RedBinAttendance(models.Model):
    ...
    status = models.CharField(
        max_length=1, choices=ATTENDANCE_CHOICES, default="", blank=True
    )
...
class MachineBreakdownIntimation(models.Model):
    ...
    repair_hours = models.DecimalField(
...
class ToolHistoryReport(models.Model):
    ...
    estimated_maintenance_frequency = models.CharField(
...
class ToolBreakdownIntimation(models.Model):
    ...
    doc_no = models.CharField(
        max_length=50, default="AOT-F-BD-01", blank=True, null=True
    )
    ...
    time_taken_to_rectify = models.CharField(
        max_length=100, blank=True, null=True
    )  # CharField रखा है ताकि '2 Hours' जैसा टेक्स्ट भी सेव हो सके
...
class FourMChangeRecord(models.Model):
    ...
    STATUS_CHOICES = [
        ("OK", "No Change"),
        ("CHANGE", "Change Implemented"),
        ("NOT_SET", "Not Set"),
    ]
    status_man = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="NOT_SET"
    )
    status_machine = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="NOT_SET"
    )
    status_material = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="NOT_SET"
    )
    status_method = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="NOT_SET"
    )
...
class OperatorObservancePlan(models.Model):
    filled_date = models.DateField(
        auto_now_add=True
    )  # Automatically tracks when the plan was submitted
...
class TigMigWelderQual(models.Model):
    ...
    welding_process = models.CharField(
        max_length=100, blank=True, null=True
    )  # MIG / TIG
    machine_no = models.CharField(max_length=100, blank=True, null=True)
    ...
    test_results = models.JSONField(
        default=dict
    )  # To save visual, defect, strength bend tests
...
class MachineEventLog(models.Model):
    ...
    event_type = models.CharField(
        max_length=50
    )  # Seedha backend se aayega (ON, OFF, etc.)
    timestamp = models.DateTimeField()
    shift = models.CharField(max_length=5)
...
class HourlyDowntimeLog(models.Model):
    ...
    specific_reason = models.CharField(
        max_length=255, default="Supervisor not save the information"
    )
...
class SpotWeldingMaintenance(models.Model):
    ...
    machine_name = models.CharField(
        max_length=100, default="SPOT WELDING M/C", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class CompressorMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="Compressor", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class LatheMachineMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="LATHE MACHINE", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class VerticalDrillMachineMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="VERTICAL DRILL MACHINE", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class SurfaceGrinderMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="SURFACE GRINDER", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class BaseGrinderMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="Base Grinder", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class BeltGrinderMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="BELT GRINDER", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class PipeCuttingMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="Pipe Cutter", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class VibraMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="Vibra", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class DipMoldingMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="Dip Molding Machine", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class ServoPressMaintenance(models.Model):
    # Meta Data Fields
    machine_name = models.CharField(
        max_length=100, default="Servo Press", blank=True, null=True
    )
    date = models.DateField()
    machine_no = models.CharField(max_length=50, blank=True, null=True)
    location = models.CharField(max_length=100, blank=True, null=True)
...
class FixtureMaintenanceRecord(models.Model):
    ...
    part_no = models.CharField(
        max_length=100, blank=True, null=True, help_text="Part Number"
    )
    ...
    checklist_data = models.JSONField(
        default=list, help_text="Stores the 8 checklist points data"
    )

    # 3. Technical Chart Data (JSON Format)

    pin_chart_data = models.JSONField(
        default=list, help_text="Stores 12 rows of Pin data"
    )
    bush_chart_data = models.JSONField(
        default=list, help_text="Stores 12 rows of Bush data"
    )
...
class IncomingMaterialInspection(models.Model):
    ...
    mtc = models.CharField(
        max_length=10, blank=True, null=True
    )  # Options: YES, NO, N/A
    ga_nga = models.CharField(
        max_length=10, blank=True, null=True
    )  # Options: GA, NGA, N/A
    coil_no = models.CharField(max_length=100, blank=True, null=True)
    invoice_no = models.CharField(max_length=100, blank=True, null=True)
    qty = models.CharField(
        max_length=50, blank=True, null=True
    )  # CharField in case of unit inclusion (e.g. '50 kgs')
...
class ReportActivityLog(models.Model):
    ...
    timestamp = models.CharField(max_length=19, default=current_datetime_string)
...
class QANotification(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="qa_notifications"
    )
...
class UserProfile(models.Model):
    ...
    contact_email = models.EmailField(
        null=True, blank=True, help_text="Alternate/Direct Email"
    )
    full_name = models.CharField(
        max_length=100, null=True, blank=True, help_text="Full Name of the User"
    )
    designation = models.CharField(
        max_length=100, null=True, blank=True, help_text="Designation of the User"
    )
    ...
    location = models.CharField(
        max_length=50, choices=LOCATION_CHOICES, default="Plant 1"
    )
    ...
    department = models.CharField(
        max_length=50, choices=DEPARTMENT_CHOICES, default="QA"
    )
